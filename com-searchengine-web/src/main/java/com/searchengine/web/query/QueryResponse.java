...
import com.searchengine.preprocess.alpha.invertedIndex.InvertedIndex;
import com.searchengine.preprocess.alpha.util.ResultGenerator;
...
public class QueryResponse {
	...
	public QueryResponse(RedisService redisService) {
		this.redisService=redisService;
		Object value=redisService.get("invertedIndexMap");
		if(value==null){
			//创建倒排索引
			InvertedIndex invertedIndex = new InvertedIndex();
			invertedIndexMap = invertedIndex.createInvertedIndex();
			Type type = new TypeToken<HashMap<String, ArrayList<String>>>() { }.getType();
			String valueStr= JsonUtils.toJson(invertedIndexMap,type);
			redisService.add("invertedIndexMap",valueStr,1, TimeUnit.HOURS);
		}else {
			String valueStr=String.valueOf(value);
			Type type = new TypeToken<HashMap<String, ArrayList<String>>>() { }.getType();
			invertedIndexMap = JsonUtils.fromJson(valueStr,type);
		}
		dictSeg = new DictSegment();
		resultGenerator = new ResultGenerator();
	}

	public ArrayList<QueryResult> getResponse(String request) {
		doQuery(request);
		return results;
	}
	...
	private void doQuery(String request) {
		...
		try {
			if (resultUrl.size() != 0) {
				...
				for (String url : resultUrl) {
					QueryResult tempR = resultGenerator.generateResult(url);
					if (tempR == null)
	...
	public static void main(String[] args) {
		QueryResponse queryResponse = new QueryResponse(null);
		ArrayList<QueryResult> results = queryResponse.getResponse("中国教育");
